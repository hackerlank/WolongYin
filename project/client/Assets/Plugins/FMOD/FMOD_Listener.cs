...
public class FMOD_Listener : MonoBehaviour 
{
	...
	{		
...
#if UNITY_ANDROID && !UNITY_EDITOR
		{
			...
			using (var www = new WWW(bankPath))
			{
				...
				{
					FMOD.Studio.UnityUtil.LogError(String.Format("Bank {0} encountered a loading error {1}", bankPath, www.error));
					result = FMOD.RESULT.ERR_EVENT_ALREADY_LOADED;
				}
				else
				{
					result = FMOD_StudioSystem.instance.System.loadBankMemory(www.bytes, FMOD.Studio.LOAD_BANK_FLAGS.NORMAL, out bank);
				}
			}
		}
#else
        {
			string bankPath = getStreamingAsset(fileName);
			result = FMOD_StudioSystem.instance.System.loadBankFile(bankPath, LOAD_BANK_FLAGS.NORMAL, out bank);
		}
#endif
    ...
    string getStreamingAsset(string fileName)
    {
        string sFileURL = "";
        string sRealFile = "";
        for (int i = 0; i < ClientSetup.Get().listWorkingPaths.Count; i++)
        {
            sFileURL = ClientSetup.Get().listWorkingPaths[i];
            sFileURL += ClientSetup.Get().GetBankTag() + fileName;
...
//         {
//             bankPath = "file:///android_asset/patchable_resources/banks";
//         }
