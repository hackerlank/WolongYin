...
public class SceneBase : Table.Binary, Table.IKey
{
	#region PosStruct
...
// source: C_场景配置表.xls, sheet: Scene
public sealed class SceneBaseManager : Table.Manager<SceneBase>
{
	private static readonly SceneBaseManager ms_instance = new SceneBaseManager();
	
	private SceneBaseManager()
	{
	}
	...
	{
		get { return "scene_base.tbl"; }
	}
	
	public bool Load(string path)
	{
		Register();
		return Load(path, source, SceneBase.Version);
	}
	
	public SceneBase Find(UInt32 key)
	{
		return FindInternal(key);
	}
	
	#region new helper
	public static object NewSceneBase()
	{
		return new SceneBase();
	}
	public static object NewSceneBasePosStruct()
	{
		return new SceneBase.PosStruct();
	}
	
	private static void Register()
	{
		Table.NewHelper.Clear();
		Table.NewHelper.Register(typeof(SceneBase), NewSceneBase);
		Table.NewHelper.Register(typeof(SceneBase.PosStruct), NewSceneBasePosStruct);
	}
	#endregion
}
